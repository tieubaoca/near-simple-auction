...
use near_sdk::{
    env, near_bindgen, AccountId, BorshStorageKey, PanicOnDefault, Promise, PromiseOrValue, Balance, ext_contract
};
#[near_bindgen]
...
impl NFTMarket {
    ...
    pub fn mint(
        ...
        assert_eq!(env::attached_deposit(), MINT_FEE, "Require 0.1N to mint NFT" );
    ...
    pub fn create_auction(
        ...
        start_price: Balance,
        start_time: u64,
        end_time: u64) -> Auction{
        let owner_id = self.tokens.owner_by_id.get(&auction_token).unwrap();
        assert_eq!(owner_id, env::predecessor_account_id(), "You not own this NFT");
        assert_eq!(self.auctioned_tokens.contains(&auction_token), false, "Already auctioned");
        assert_eq!(env::attached_deposit(), CREATE_AUCTION_FEE, "Require 1N to create an auction");
        self.tokens.internal_transfer(&env::predecessor_account_id(), &env::current_account_id(), &auction_token, None, None);
        let mut auction_ids: Vector<u128>;
        if self.auctions_by_owner.get(&env::predecessor_account_id()).is_none() {
            auction_ids = Vector::new(b"auction_ids".to_vec()); 
        }
        else{
            auction_ids = self.auctions_by_owner.get(&env::predecessor_account_id()).unwrap();
        }
        ...
        self.auctions_by_owner.insert(&env::predecessor_account_id(), &auction_ids);
    ...
    pub fn bid(&mut self, auction_id: u128) {
        ...
        assert_eq!(env::block_timestamp()  > auction.start_time, true, "This auction has not started");
        assert_eq!(env::block_timestamp() < auction.end_time, true, "This auction has already done");
        assert_eq!(env::attached_deposit()> auction.current_price, true, "Price must be greater than current winner's price");
    ...
    pub fn claim_nft(&mut self, auction_id: u128) {
        ...
        assert_eq!(env::block_timestamp() > auction.end_time, true, "The auction is not over yet");
        assert_eq!(env::predecessor_account_id(), auction.winner, "You are not the winner");
        assert_eq!(auction.clone().is_nft_claimed, false, "You has already claimed NFT");
        self.tokens.internal_transfer_unguarded(&auction.auction_token, &env::current_account_id(), &auction.winner );
    ...
    pub fn claim_near(&mut self, auction_id: u128) {
        ...
        assert_eq!(env::predecessor_account_id(), auction.owner, "You are not operator of this auction");
        assert_eq!(env::block_timestamp() > auction.end_time, true, "The auction is not over yet");
    ...
    pub fn claim_back_nft(&mut self, auction_id: u128) {
        ...
        self.tokens.internal_transfer_unguarded(&auction.auction_token, &env::current_account_id(), &auction.owner );
        auction.is_nft_claimed = true;
        self.auctioned_tokens.remove(&auction.auction_token);
        self.auction_by_id.insert(&auction_id, &auction);
    }
...
mod tests {
    ...
    fn get_context(account_id: String, current_account_id: String, storage_usage: u64, block_timestamp: u64,attached_deposit: Balance) -> VMContext {
        VMContext {
            current_account_id,
    ...
    fn test_mint_nft() {
        ...
        assert_eq!(contract.nft_token(String::from("1")).unwrap().owner_id, bob().to_string(), "Owner must be bob");
    }
    ...
    fn test_auction_panic_not_own_nft(){
        ...
        testing_env!(get_context(senna().into(), senna().into(), 0,0,MINT_FEE));
        contract.mint(String::from("1"),bob(),Some(nft("first")));
        testing_env!(get_context(senna().into(), senna().into(), 0,0,CREATE_AUCTION_FEE));
        contract.create_auction(String::from("1"), 1_000_000_000_000_000_000_000_000, 100, 3700);
    ...
    fn test_auction_panic_bid_too_early(){
        ...
        testing_env!(get_context(alice().into(), senna().into(), 0,0,CREATE_AUCTION_FEE));
        contract.create_auction(String::from("1"), 1_000_000_000, 1000, 4600);
        testing_env!(get_context(bob().into(), senna().into(), 0,50,1_500_000_000_000_000_000_000_000));
        //contract.bid(0);
    }
}
